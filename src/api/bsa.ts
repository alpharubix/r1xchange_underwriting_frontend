...
export async function getDateRange(
  ...
  const response = await apiClient.post<DateRangeResponse>("/bsa/report-date-range",
  {
...
export async function getBankAccounts():Promise<BankAccounts[]>{
  ...
  const response = await apiClient.get<BankAccountsResponse>("/bsa/bank-accounts",{
    // params:{cust_id:cust_id |null},
    errorMessage : "Failed to fetch the bank Accounts . Please Try again ! {PRATHAM}"
  })
...
export async function getBsaBankAccounts(): Promise<BsaBankAccount[]> {
  const response = await apiClient.get<BsaBankAccountsResponse>("/crm/accounts-filter", {
    params: { module: "bsa" },
