...
export async function checkLendingEligibility(
  options?: RequestOptions
): Promise<{
  eligible: LendingEligibilityData[];
  ineligible: LendingEligibilityData[];
}> {
  ...
  const mapBank = (bank: any): LendingEligibilityData => ({
    ...
    passed_parameters: (bank.passed_parameters || []).map((p: any) => ({
      parameter: p,
    })),
    failed_parameters: (bank.reason || []).map((r: any) => ({
      ...
      expected:
        r.lender_val !== undefined && r.lender_val !== null
          ? `${r.operator} ${r.lender_val}`
          : undefined,
    })),
  });
  ...
  if (Array.isArray(resData)) {
    ...
    return {
      eligible: mapped
        .filter((b) => b.eligibility_score > 50)
        .sort((a: any, b: any) => b.eligibility_score - a.eligibility_score),
      ineligible: mapped
        .filter((b) => b.eligibility_score <= 50)
        .sort((a: any, b: any) => b.eligibility_score - a.eligibility_score),
    };
    ...
    resData &&
    Array.isArray(resData.eligible_banks) &&
    Array.isArray(resData.rejected_banks)
  ) {
    return {
      eligible: resData.eligible_banks
        .map(mapBank)
        .sort((a: any, b: any) => b.eligibility_score - a.eligibility_score),
      ineligible: resData.rejected_banks
        .map(mapBank)
        .sort((a: any, b: any) => b.eligibility_score - a.eligibility_score),
    };
  }
