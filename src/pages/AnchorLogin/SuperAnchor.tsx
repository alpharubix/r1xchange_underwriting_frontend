...
export default function SuperAnchor({
  anchorsList,
  onViewCustomers,
  onSelectAnchor,
}: SuperAnchorProps) {
  ...
  const handleCreateAnchor = (e: React.FormEvent) => {
    ...
    if (
      !newAnchorForm.name ||
      !newAnchorForm.phone ||
      !newAnchorForm.email_id ||
      !newAnchorForm.password
    ) {
  ...
  const filteredAnchors = useMemo(() => {
    return anchorsList.filter((anc) => {
      const matchesName =
        !filterName ||
        anc.anchor_name.toLowerCase().includes(filterName.toLowerCase());
      const matchesCode =
        !filterCode ||
        anc.anchor_code.toLowerCase().includes(filterCode.toLowerCase());
      const matchesLoginId =
        !filterLoginId ||
        anc.login_id.toLowerCase().includes(filterLoginId.toLowerCase());

      const statusStr = anc.is_active ? 'active' : 'inactive';
      const matchesStatus =
        !filterStatus || statusStr === filterStatus.toLowerCase();
      ...
      const matchesUpdatedBy =
        ...
        (anc.updated_by || '')
          .toLowerCase()
          .includes(filterUpdatedBy.toLowerCase());
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#8a8d97] font-medium mt-0.5">
            Manage and view all registered User organizations
          </p>
        </div>
        <button
            ...
            <table className="w-full text-left border-collapse text-sm whitespace-nowrap">
              <thead>
                <tr className="border-b border-[#F0F1F5] text-[#A0A3AD] font-bold text-[10.5px] uppercase tracking-wider bg-[#F6F6F8]/30">
                  <th className="py-3 px-4 font-bold text-[#A0A3AD]">
                    User Name
                  </th>
                  <th className="py-3 px-4 font-bold text-[#A0A3AD]">
                    User Code
                  ...
                  <th className="py-3 px-4 font-bold text-center text-[#A0A3AD]">
                    Action
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#F6F6F8] text-[13px] text-[#3A3C46]">
                {filteredAnchors.map((anc, idx) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-4 font-mono font-bold text-[#002366]">
                      {anc.anchor_code}
                    </td>
                    ...
                    <td className="py-4 px-4 text-[#3A3C46] text-xs">
                      {anc.created_at
                        ? new Date(anc.created_at).toLocaleString()
                        : '-'}
                    </td>
                    <td className="py-4 px-4 text-[#3A3C46] text-xs">
                      {anc.updated_at
                        ? new Date(anc.updated_at).toLocaleString()
                        : '-'}
                    </td>
                    <td className="py-4 px-4 text-[#3A3C46] text-xs truncate max-w-[120px]">
                      {anc.created_by || '-'}
                    </td>
                    <td className="py-4 px-4 text-[#3A3C46] text-xs truncate max-w-[120px]">
                      {anc.updated_by || '-'}
                    ...
                    >
                      <button
                        onClick={() =>
                          onViewCustomers?.(anc._id || anc.anchor_code)
                        }
              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5 font-medium">
                    Create a new user organization profile
                  </p>
                </div>
                <button
                  onClick={() => setIsNewAnchorModalOpen(false)}
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setShowNewAnchorPassword(!showNewAnchorPassword)
                        }
