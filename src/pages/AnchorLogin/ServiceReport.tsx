...
export default function ServiceReport({
  ...
  const [viewingSaveMoneyReport, setViewingSaveMoneyReport] = useState<
    any | null
  >(null);
  const [viewingRectifyMoneyReport, setViewingRectifyMoneyReport] = useState<
    any | null
  >(null);
  ...
  const handleCreateReport = async (
    ...
    try {
      ...
      if (res.data.is_balance_available) {
        ...
        toast.error(
          `Insufficient wallet balance for ${moduleName}. Please select a payer to proceed.`
        );
      ...
      console.error(error);
      toast.error(
        'Failed to check wallet balance. Proceeding to payer selection.'
      );
  ...
  const handleCustomerPay = async () => {
    ...
    try {
      ...
      toast.success(
        `Payment order created. The customer can now pay for ${config.moduleName} from their dashboard.`
      );
  ...
  const { data: gstReports = [], isLoading: isGstLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.map((item: any) => ({
        ...
        completed:
          item.completed !== undefined ? String(item.completed) : 'Yes',
      }));
    },
    ...
    queryFn: async () => {
      ...
      return data.map((item: any) => ({
        ...
        reportId:
          ...
          item.reportId ||
          item.reference_id ||
          item.itr_reference_id ||
          '',
        itrFromDate:
          item.itr_from_date || item.itrFromDate || item.from_date || '',
    ...
    queryFn: async () => {
      ...
      return data.map((item: any) => ({
        ...
        reportId:
          ...
          item.reportId ||
          item.reference_id ||
          item.cibil_reference_id ||
          '',
        cibilFromDate:
          item.cibil_from_date || item.cibilFromDate || item.from_date || '',
        cibilToDate:
          item.cibil_to_date || item.cibilToDate || item.to_date || '',
        generatedOn: formatDateTime(
          item.created_at ||
            item.generatedOn ||
            item.generated_on ||
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-['Space_Grotesk'] text-2xl font-bold text-[#1D1E2C]">
              Services & Reports
            </h1>
            <p className="text-sm text-[#8a8d97] font-medium">
              Manage Customer services and view reports
            </p>
          </div>
        </div>
      </div>

      <div className="flex items-center gap-4 bg-[#002366] p-5 rounded-2xl shadow-md shadow-[#002366]/20">
        <div className="h-14 w-14 rounded-full bg-white/10 text-white flex items-center justify-center font-bold text-xl shrink-0 tracking-wider">
          {getCustomerInitials(
        ...
        <div>
          ...
          <p className="text-xs text-slate-300 mt-1 font-medium flex flex-wrap items-center">
            <span>ID : {selectedCustomer.id}</span>
            <span className="px-2 text-slate-400">|</span>
            <span>
              Phone :{' '}
              <span className="font-bold text-white">
                {selectedCustomer.phone || selectedCustomer.phone_no}
              </span>
            </span>
            <span className="px-2 text-slate-400">|</span>
            <span>
        ...
        {(
          ...
          return (
            <button
              ...
              className={`-ml-1 inline-flex items-center gap-2 px-4 py-2 rounded-3xl text-sm font-semibold transition-all duration-200 cursor-pointer ${isActive ? 'bg-[#002366] text-white shadow-sm shadow-[#002366]/25' : 'bg-[#f0f4f9] text-slate-600 hover:bg-[#e4ebf5] hover:text-slate-900'}`}
            >
              <span
        ...
        <AnimatePresence mode="wait" custom={slideDirection} initial={false}>
          ...
          >
                ...
                <div className="space-y-6 animate-in fade-in duration-200">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Goods and Services Tax Reports & Filings
                      </p>
                    </div>
                    <Button
                      onClick={() =>
                        handleCreateReport('GST', 'GST', 561, () =>
                              ...
                              gstReports.map((report) => (
                                ...
                                >
                                  ...
                                  <td className="py-4 px-4 text-slate-600">
                                    {formatDateOnly(report.gstFromDate)}
                                  </td>
                                  <td className="py-4 px-4 text-slate-800">
                                    {formatDateOnly(report.gstToDate)}
                                  </td>
                                  <td className="py-4 px-4 text-slate-600">
                                    ...
                                    <div className="flex items-center justify-center gap-2">
                                      <button
                                        ...
                                        onClick={() => {
                                          ...
                                          localStorage.setItem(
                                            'selected_gst_reference_id',
                                            report.reportId || ''
                                          );
                                          localStorage.setItem(
                                            'selected_gst_from_date',
                                            report.gstFromDate || ''
                                          );
                                          localStorage.setItem(
                                            'selected_gst_to_date',
                                            report.gstToDate || ''
                                          );
              ...
              (viewingItrReport ? (
                ...
                <div className="space-y-6 animate-in fade-in duration-200">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <Button
                      onClick={() =>
                        handleCreateReport('ITR', 'ITR', 525, () =>
                ...
                <div className="space-y-6 animate-in fade-in duration-200">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Credit Bureau Score & Report Details
                      </p>
                    </div>
                    <Button
                      onClick={() =>
                        handleCreateReport('CIBIL', 'CIBIL', 643, () =>
                ...
                <div className="space-y-6 animate-in fade-in duration-200">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Select a CIBIL report to analyze active accounts for
                        savings
                      </p>
                    </div>
                  </div>
                                  ...
                                  <td className="py-4 px-4 text-center">
                                    <div className="flex items-center justify-center gap-2">
                                      <button
                                        type="button"
                                        onClick={() =>
                                          setViewingSaveMoneyReport(report)
                                        }
                ...
                <div className="space-y-6 animate-in fade-in duration-200">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        Select a CIBIL report to analyze overdue accounts and
                        DPD
                      </p>
                    </div>
                  </div>
                                  ...
                                  <td className="py-4 px-4 text-center">
                                    <div className="flex items-center justify-center gap-2">
                                      <button
                                        type="button"
                                        onClick={() =>
                                          setViewingRectifyMoneyReport(report)
                                        }
                                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200 text-xs font-semibold text-[#1D1E2C] hover:border-[#1D1E2C] hover:bg-[#1D1E2C]/5 transition-colors shadow-sm bg-white"
            ...
            {reportsSubTab === 'access_money' && (
              <AccessMoneyReportView selectedCustomer={selectedCustomer} />
            )}
            {reportsSubTab === 'wallet' && (
              <WalletModal selectedCustomer={selectedCustomer} />
            )}
          </motion.div>
        </AnimatePresence>
      </div>
