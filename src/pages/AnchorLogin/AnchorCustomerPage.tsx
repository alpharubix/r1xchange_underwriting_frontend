...
export default function AnchorCustomerPage() {
  ...
  const userRole = String(
    user?.role || localStorage.getItem('user_role') || 'anchor'
  ).toLowerCase();
  const isSuperAnchor =
  ...
  const [selectedAnchorFilter, setSelectedAnchorFilter] = useState<
    string | null
  >(null);
  const [selectedAnchor, setSelectedAnchor] = useState<any | null>(null);
  const [superAnchorViewMode, setSuperAnchorViewMode] = useState<
    'list' | 'details'
  >('list');
  ...
  const handleCreateCustomer = (e: React.FormEvent) => {
    ...
    if (
      ...
      !newCustomerForm.password
    ) {
      toast.error(
        'Please fill in all required fields (Name, Mobile, Email, Password).'
      );
      return;
    }
    ...
    if (isSuperAnchor) {
      const chosenAnchor = anchorsList.find(
        (a) => a._id === newCustomerForm.anchor_id
      );
  ...
  const filteredCustomers = useMemo(() => {
    ...
    return customers.filter((c) => {
      ...
      const matchesCustomerName =
        !filterCustomerName ||
        String(c.name || c.customer_name || '')
          .toLowerCase()
          .includes(filterCustomerName.toLowerCase());
      const matchesCustomerPhone =
        !filterPhoneNo ||
        String(c.phone || c.phone_no || '')
          .toLowerCase()
          .includes(filterPhoneNo.toLowerCase());
      ...
      const matchesGstNo =
        ...
        String(c.gst_no || '')
          .toLowerCase()
          .includes(filterGSTNo.toLowerCase());
      const matchesStatus =
        !filterStatus ||
        String(c.status || '')
          .toLowerCase()
      ...
      const matchesAnchor =
        !isSuperAnchor ||
        !effectiveAnchorFilter ||
        c.anchor_id === effectiveAnchorFilter ||
        c.anchor_code === effectiveAnchorFilter;
  ...
  const renderStatusOrValue = (val: any) => {
    if (
      ...
      return isTrue ? (
        <span
          className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-[#EAF9F0] text-[#2E9B5C]"
        ...
        <span
          className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-[#FDEEEE] text-[#E5484D]"
          title="Pending / False"
        >
    ...
    return (
      <span
        className={`font-bold ${num > 0 ? 'text-[#2E9B5C]' : 'text-[#E5484D]'}`}
      >
              ...
              {superAnchorViewMode === 'list' ? (
                <SuperAnchor
                  ...
                  onViewCustomers={(anchorId) => {
                    const anchorObj = anchorsList.find(
                      (a) => a._id === anchorId || a.anchor_code === anchorId
                    );
                    setSelectedAnchor(
                      anchorObj || { _id: anchorId, anchor_name: anchorId }
                    );
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h1 className="text-2xl font-extrabold tracking-tight text-slate-900">
                        {selectedAnchor?.anchor_name || 'Anchor Details'}
                      </h1>
                      <p className="text-sm text-slate-500 mt-0.5">
                        Organization code: {selectedAnchor?.anchor_code}
                      </p>
                    </div>
                  </div>
                    ...
                    <CardContent className="p-8 space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                          ...
                          <span className="text-base font-mono font-extrabold text-[#002366] mt-1 block">
                            {selectedAnchor?.anchor_code || '-'}
                          </span>
                        </div>
                        <div>
                          <span className="text-[10px] uppercase font-bold text-slate-400 block tracking-wider">
                          ...
                          <span className="text-base font-mono font-medium text-slate-700 mt-1 block">
                            {selectedAnchor?.login_id || '-'}
                          </span>
                        </div>
                        <div>
                          <span className="text-[10px] uppercase font-bold text-slate-400 block tracking-wider">
                ...
                <div className="flex flex-col items-center justify-center p-12 text-center bg-white border border-gray-100 rounded-3xl space-y-4 shadow-sm min-h-[350px]">
                  ...
                  <div className="max-w-md space-y-2">
                    ...
                    <p className="text-sm text-slate-500 font-medium">
                      Please select an Anchor organization from the Anchors tab
                      to view their registered users.
                    </p>
                  </div>
                  <Button
                    onClick={() => setActiveTab('anchor')}
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="border-b border-gray-100 text-gray-400 font-bold text-[11px] uppercase tracking-wider">
                            <th className="py-3 px-4 font-bold text-gray-400">
                              ACCOUNT ID
                            </th>
                            <th className="py-3 px-4 font-bold text-gray-400">
                              CUSTOMER NAME
                            ...
                            <th className="py-3 px-4 font-bold text-center text-gray-400">
                              BSA
                            </th>
                            <th className="py-3 px-4 font-bold text-center text-gray-400">
                              GST
                      ...
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                        (page) => (
                          <button
                            ...
                            className={`h-9 w-9 rounded-xl border text-sm font-semibold transition-all cursor-pointer ${currentPage === page ? 'bg-[#002366] border-[#002366] text-white shadow-sm shadow-[#002366]/20' : 'border-gray-200 hover:bg-gray-50 text-gray-600'}`}
                          >
                            {page}
                          </button>
                        )
              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5 font-medium">
                    Create a new customer profile and allocate audit credits
                  </p>
                </div>
                ...
                {isSuperAnchor && (
                  <div className="space-y-1">
                    ...
                    >
                      <SelectTrigger
                        id="cust-anchor"
                        className="w-full h-10 px-3 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 bg-white focus:ring-[#002366] outline-none"
                      >
                      ...
                      <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center justify-center">
                        ...
                        >
                          <button
                            type="button"
                            onClick={() =>
                              setShowNewCustPassword(!showNewCustPassword)
                            }
