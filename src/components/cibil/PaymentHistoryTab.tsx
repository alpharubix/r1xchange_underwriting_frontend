...
const glossaryItems = [
  {
    code: '000',
    label: 'No Delays',
    desc: 'Payments are being timely made with zero days past due.',
    color: 'bg-emerald-50 text-emerald-700 border-emerald-200',
  },
  {
    code: 'XXX',
    label: 'Not Reported',
    desc: 'Payment information has not been reported to CIBIL by the lender.',
    color: 'bg-slate-100 text-slate-500 border-slate-200',
  },
  {
    code: 'STD',
    label: 'Standard Account',
    desc: 'Payments are being made within 90 days of due date.',
    color: 'bg-indigo-50 text-indigo-700 border-indigo-200',
  },
  {
    code: 'NEW',
    label: 'New Account',
    desc: 'The credit facility was opened during this month.',
    color: 'bg-blue-50 text-blue-700 border-blue-200',
  },
  {
    code: '01+',
    label: 'Overdue DPD',
    desc: 'Represents days past due (e.g. 01+, 30, 90 days late).',
    color: 'bg-red-50 text-red-700 border-red-200',
  },
  {
    code: 'SPM / SMA',
    label: 'Special Mention',
    desc: 'Special Mention Account showing early signs of distress.',
    color:
      'bg-orange-50 text-orange-700 border-orange-200 font-semibold animate-pulse',
  },
  {
    code: 'CLSD',
    label: 'Closed Account',
    desc: 'The credit account has been closed by the lender.',
    color: 'bg-slate-200 text-slate-600 border-slate-300 font-semibold',
  },
];
...
export default function PaymentHistoryTab({
  ...
  data: CibilPaymentHistoryData;
}) {
  const [historySegment, setHistorySegment] = useState<'Active' | 'Closed'>(
    'Active'
  );
  ...
  const getMonthValue = (row: any, monthName: string) => {
    ...
    if (row[monthName] !== undefined && row[monthName] !== null)
      return String(row[monthName]);
    if (row[m] !== undefined && row[m] !== null) return String(row[m]);

    // Custom variations
    if (m === 'jun' || m === 'june')
      return String(row.Jun ?? row.jun ?? row.june ?? '-');
    if (m === 'jul' || m === 'july')
      return String(row.Jul ?? row.jul ?? row.july ?? '-');
    if (m === 'sept' || m === 'sep')
      return String(row.Sept ?? row.sep ?? row.sept ?? '-');
        ...
        <div className="inline-flex rounded-xl bg-slate-100 p-1.5 border border-slate-200">
          {(['Active', 'Closed'] as const).map((segment) => (
            <button
              ...
              className={`rounded-lg px-6 py-2.5 text-sm font-bold transition-all cursor-pointer ${historySegment === segment ? 'bg-[#002366] text-white shadow-md shadow-[#002366]/20' : 'text-slate-500 hover:text-slate-800'}`}
            >
              {segment} Timeline
            </button>
          ))}
      ...
      <div className="space-y-6">
        {selectedTracks.length > 0 ? (
          selectedTracks.map((track: any, idx: number) => {
            ...
            const limit =
              track.sanctionedAmount ??
              track.sanctionedAmountCreditLimit ??
              track.santionedamountCreditLimit ??
              0;
            const date =
              track.date ??
              track.reportedDate ??
              track.reportedandCertified ??
              'N/A';
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-slate-100 pb-4 gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs">
                    <div>
                      ...
                      <strong className="ml-1.5 font-bold text-slate-800">
                        {formatINR(limit)}
                      </strong>
                    </div>
                    ...
                    <div>
                      <span className="text-slate-400">
                        {historySegment === 'Active'
                          ? 'Reported Date:'
                          : 'Closed Date:'}
                      </span>
                      <strong className="ml-1.5 font-semibold text-slate-800">
                        {date}
                      </strong>
                    </div>
                  </div>
                ...
                <div className="overflow-x-auto">
                  <div className="min-w-[800px] space-y-2">
                    ...
                    <div
                      ...
                      style={{
                        display: 'grid',
                        gridTemplateColumns: '100px repeat(12, minmax(0, 1fr))',
                      }}
                    >
                    ...
                    {history.length > 0 ? (
                      ...
                      <div className="py-6 text-center text-xs text-slate-400">
                        No repayment history records returned.
                      </div>
                    )}
                  </div>
                </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {glossaryItems.map((item, idx) => (
            <div
              key={idx}
              className="flex gap-3 p-3 rounded-xl border border-slate-50 bg-slate-50/50"
            >
              <span
                className={`inline-flex items-center justify-center rounded-lg border h-10 w-16 text-xs font-bold shrink-0 ${item.color}`}
              >
                {item.code}
              </span>
              <div>
