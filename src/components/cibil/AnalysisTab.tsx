      ...
      Object.entries(analysisData).forEach(([loanType, val]: [string, any]) => {
        ...
        if (loanType !== 'comments' && val && typeof val === 'object') {
          parsed.push({
            loanType,
            frequency:
              val.FrequencyofCreditFacilitytaken ?? val.frequency ?? '-',
            frequencyLast12Months:
              val.frequencyofCreditFacilitytakeninlast12months ??
              val.frequencyLast12Months ??
              '-',
            delayedActive:
              val.percentofdelayinActiveAccount ?? val.delayedActive ?? '-',
            delayedClosed:
              val.percentofdelayinClosedAccount ?? val.delayedClosed ?? '-',
            delayedTotal: val.percentofdelaytotal ?? val.delayedTotal ?? '-',
            delayedLast12Months:
              val.percentofdelayinlast12months ??
              val.delayedLast12Months ??
              '-',
            changeLast12Months:
              val.changeindefaultinlast12months ??
              val.changeLast12Months ??
              '-',
          });
        }
      });
  ...
  const comments = useMemo(() => {
    ...
    if (list.length === 0) {
      const totalOverdue = Number(
        generalInfo.accountsSummary?.totalOverdueAmount ?? 0
      );
      const totalOutstanding = Number(
        generalInfo.accountsSummary?.totalOutstandingBalance ?? 0
      );
          ...
          <table className="w-full text-left text-sm">
            <thead>
              ...
              <tr className="bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500 border-b border-slate-200">
                <th rowSpan={3} className="px-4 py-3 align-middle">
                  Loan Type
                </th>
                <th
                  rowSpan={3}
                  className="px-4 py-3 align-middle text-center max-w-[120px]"
                >
                  Total Facilities
                </th>
                <th
                  ...
                  className="px-4 py-3 align-middle text-center max-w-[120px]"
                >
                  Facilities in Last 12m
                </th>
                <th
                  colSpan={4}
                  className="px-4 py-1.5 text-center border-b border-slate-200"
                >
                  Delayed Payment Percentage
                </th>
                <th
                  rowSpan={3}
                  className="px-4 py-3 align-middle text-center max-w-[120px]"
                >
                  Default Change Last 12m
                </th>
              </tr>
              {/* Row 2 Headers */}
              <tr className="bg-slate-50 text-[9px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100">
                ...
                <th rowSpan={2} className="px-2 py-1 text-center align-middle">
                  Last 12m
                </th>
              </tr>
              {/* Row 3 Headers */}
              <tr className="bg-slate-50 text-[8px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-200">
                <th className="px-2 py-1 text-center border-r border-slate-100">
                  Active
                </th>
                <th className="px-2 py-1 text-center border-r border-slate-100">
                  Closed
            ...
            <tbody className="divide-y divide-slate-100">
              {loanTypes.length > 0 ? (
                loanTypes.map((row: any, idx: number) => {
                  const isTotalRow =
                    String(row.loanType).toLowerCase().trim() === 'total';
                  const activeDelay = String(row.delayedActive || '')
                    .replace('%', '')
                    .trim();
                  const totalDelay = String(row.delayedTotal || '')
                    .replace('%', '')
                    .trim();

                  // Highlight red if delays exist
                  const hasDelays =
                    (activeDelay && Number(activeDelay) > 0) ||
                    (totalDelay && Number(totalDelay) > 0);

                  return (
                    <tr
                      key={idx}
                      className={`hover:bg-slate-50/50 ${isTotalRow ? 'bg-slate-50/80 font-bold border-t-2 border-slate-200' : ''}`}
                    >
                      ...
                      <td className="px-4 py-3 text-center text-slate-600 font-medium">
                        {row.frequencyLast12Months || '-'}
                      </td>

                      {/* Delayed payment cols */}
                      <td
                        className={`px-2 py-3 text-center font-bold ${hasDelays && activeDelay ? 'text-red-500 bg-red-50/20' : 'text-slate-600'}`}
                      >
                        {row.delayedActive || '-'}
                      </td>
                      ...
                      <td
                        className={`px-2 py-3 text-center font-bold ${hasDelays && totalDelay ? 'text-red-500 bg-red-50/20' : 'text-slate-600'}`}
                      >
                        {row.delayedTotal || '-'}
                      </td>
                      <td className="px-2 py-3 text-center text-slate-600 font-medium">
                        {row.delayedLast12Months || '-'}
                      </td>

                      <td className="px-4 py-3 text-center text-slate-600 font-medium">
                        {row.changeLast12Months || '-'}
                      </td>
                    </tr>
                  );
                })
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {comments.length > 0 ? (
            comments.map((comment: any, idx: number) => {
              const isDefaultWarning =
                String(comment.title).toLowerCase().includes('default') ||
                String(comment.title).toLowerCase().includes('warning');
              return (
                <div
                  key={idx}
                  className={`rounded-xl border p-4 flex gap-3 ${isDefaultWarning ? 'border-red-100 bg-red-50/30 text-red-950' : 'border-slate-100 bg-slate-50/50 text-slate-900'}`}
                >
                  <div
                    className={`rounded-lg p-2 shrink-0 h-10 w-10 flex items-center justify-center ${isDefaultWarning ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-700'}`}
                  >
                    {isDefaultWarning ? (
            ...
            <p className="text-xs text-slate-400 col-span-2 py-4 text-center">
              No commentary generated for this report.
            </p>
          )}
        </div>
      </div>
