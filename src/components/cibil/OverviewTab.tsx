...
export default function OverviewTab({ data }: { data: CibilOverviewData }) {
  ...
  const totalAccounts =
    accSummary.totalAccounts ?? bureauAnalysis.totalAccounts ?? 0;
  const activeAccounts =
    accSummary.activeAccount ??
    accSummary.activeAccounts ??
    bureauAnalysis.activeAccounts ??
    0;
  const overdueAccounts =
    accSummary.overdueAccount ??
    accSummary.overdueAccounts ??
    bureauAnalysis.overdueAccounts ??
    0;
  const totalOverdueAmount =
    accSummary.totalOverdueAmount ?? bureauAnalysis.totalOverdueAmount ?? 0;
  const totalOutstandingBalance =
    accSummary.totalOutstandingBalance ??
    bureauAnalysis.totalOutstandingBalance ??
    0;
  const maximumAmountSanctioned =
    accSummary.maximumamountSactioned ??
    accSummary.maximumAmountSanctioned ??
    bureauAnalysis.maximumAmountSanctioned ??
    0;
  const oldestAccountOpenDate =
    accSummary.oldestAccountOpenDate ??
    bureauAnalysis.oldestAccountOpenDate ??
    'N/A';
  const recentAccountOpenDate =
    accSummary.recentAccountOpenDate ??
    bureauAnalysis.recentAccountOpenDate ??
    'N/A';
  const totalAccountAsAGuarantor =
    accSummary.totalAccountasaGuarantor ??
    accSummary.totalAccountAsAGuarantor ??
    bureauAnalysis.totalAccountAsAGuarantor ??
    0;

  // Repayment / Obligation stats
  const oblig = generalInfo.repaymentObligations || {};
  const totalNoOfDpdsRecorded =
    oblig.totalnoofDPDsrecorded ??
    oblig.totalNoOfDpdsRecorded ??
    bureauAnalysis.totalNoOfDpdsRecorded ??
    0;
  const dpdsInActiveAccountsCount =
    oblig.dpdsinActiveAccounts ??
    oblig.dpdsInActiveAccountsCount ??
    bureauAnalysis.dpdsInActiveAccountsCount ??
    0;
  const highestDpdRecordedInActiveAccount =
    oblig.highestDPDrecordedinActiveAccount ??
    oblig.highestDpdRecordedInActiveAccount ??
    bureauAnalysis.highestDpdRecordedInActiveAccount ??
    '000';
  const highestDpdRecordedOverall =
    oblig.highestDPDrecordedOverall ??
    oblig.highestDpdRecordedOverall ??
    bureauAnalysis.highestDpdRecordedOverall ??
    '000';
  const totalWrittenOff =
    oblig['totalWritten-off'] ??
    oblig.totalWrittenOff ??
    bureauAnalysis.totalWrittenOff ??
    0;
  const totalSettlement =
    oblig.totalSettlement ?? bureauAnalysis.totalSettlement ?? 0;
  const totalNoOfSubStdRecorded =
    oblig.totalNoOfSUBSTDRecorded ??
    oblig.totalNoOfSubStdRecorded ??
    bureauAnalysis.totalNoOfSubStdRecorded ??
    0;

  // Lender details
  const lenderDetailsList =
    generalInfo.accountDetails ?? (cibilReport as any).lenderDetails ?? [];

  // Enquiries
  const enquiryData =
    generalInfo.enquiries ?? (cibilReport as any).enquiries ?? {};
  const totalEnquiries = enquiryData.totalEnquiries ?? 0;
  const recentDateOfEnquiry =
    enquiryData.recentDate ?? enquiryData.recentDateOfEnquiry ?? 'N/A';
  ...
  const accountTypeData =
    (generalInfo.accountType ?? (cibilReport as any).accountTypeDistribution) ||
    {};
  const parsedDistribution = useMemo(() => {
    let list: any[] = [];
    if (
      typeof accountTypeData === 'object' &&
      !Array.isArray(accountTypeData)
    ) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col items-center justify-center text-center">
          ...
          <div className="relative flex flex-col items-center justify-center w-64 h-36 mt-4">
            <svg viewBox="0 0 256 144" className="w-full h-full">
              ...
              <path
                ...
                strokeDashoffset={
                  314.16 - (314.16 * scoreMetrics.percent) / 100
                }
            ...
            <div className="absolute bottom-0 flex flex-col items-center justify-center">
              <span
                className={`text-4xl font-extrabold tracking-tight ${score >= 700 ? 'text-emerald-500' : score >= 600 ? 'text-amber-500' : 'text-red-500'}`}
              >
                {score}
              </span>
          ...
          <div className="mt-4">
            <span
              className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wide ${scoreMetrics.status === 'Excellent' ? 'bg-emerald-50 text-emerald-700' : scoreMetrics.status === 'Good' ? 'bg-green-50 text-green-700' : scoreMetrics.status === 'Fair' ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'}`}
            >
              {scoreMetrics.status}
            </span>
            <p className="mt-2 text-xs text-slate-500 max-w-xs">
              {scoreMetrics.description}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span
              className={`text-2xl font-bold ${Number(overdueAccounts) > 0 ? 'text-red-600' : 'text-slate-800'}`}
            >
              {overdueAccounts}
            </span>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="flex items-baseline justify-between mt-2">
            <span
              className={`text-2xl font-bold ${Number(totalOverdueAmount) > 0 ? 'text-red-600' : 'text-slate-800'}`}
            >
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              ...
              <tbody className="divide-y divide-slate-50">
                {lenderDetailsList.length > 0 ? (
                  lenderDetailsList.map((item: any, idx: number) => {
                    const accountsCount =
                      item.noofAccounts ?? item.noOfAccounts ?? 1;
                    const amt =
                      item.totalAmountSanctioned ??
                      item.totalAmountSanctioned ??
                      0;
                    return (
                      <tr key={idx} className="hover:bg-slate-50/50">
                        <td className="py-2.5 font-semibold text-slate-800 pr-3 max-w-xs truncate">
                          {item.lender}
                        </td>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
              <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
                DPDs in Active Accounts
              </span>
              <p
                className={`mt-1 text-2xl font-extrabold ${Number(dpdsInActiveAccountsCount) > 0 ? 'text-red-500' : 'text-slate-800'}`}
              >
            ...
            <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
              <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">
                Highest DPD in Active Account
              </span>
              <p className="mt-1 text-xl font-extrabold text-slate-800">
                {highestDpdRecordedInActiveAccount || '000'}
              </p>
            </div>
          ...
          <div className="mt-5 border-t border-slate-100 pt-4 grid grid-cols-3 gap-3 text-center text-xs">
            <div className="border-r border-slate-100 pr-2">
              ...
              <span
                className={`mt-1 block font-bold ${Number(totalWrittenOff) > 0 ? 'text-red-500' : 'text-slate-700'}`}
              >
                {totalWrittenOff}
              </span>
            </div>
            <div className="border-r border-slate-100 pr-2">
              ...
              <span
                className={`mt-1 block font-bold ${Number(totalSettlement) > 0 ? 'text-amber-500' : 'text-slate-700'}`}
              >
                {totalSettlement}
              </span>
            </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="text-center py-4 border-b border-slate-100">
            ...
            <p className="text-xs text-slate-500 mt-2">
              Recent Enquiry: <strong>{recentDateOfEnquiry}</strong>
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mt-4 text-center">
            <div className="rounded-lg bg-slate-50 p-2 border border-slate-100">
              ...
              <span className="mt-1 block font-bold text-slate-800 text-sm">
                {enquiries30Days}
              </span>
            </div>
            <div className="rounded-lg bg-slate-50 p-2 border border-slate-100">
