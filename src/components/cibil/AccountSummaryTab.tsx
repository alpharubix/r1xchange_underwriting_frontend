...
export default function AccountSummaryTab({ data }: { data: any }) {
  const [activeSegment, setActiveSegment] = useState<'Active' | 'Closed'>(
    'Active'
  );
    ...
    return {
      ...
      sanctionedAmount: Number(
        acc.sanctionedAmount ??
          acc.sanctionedAmountCreditLimit ??
          acc.santionedamountCreditLimit ??
          0
      ),
      delayCount: Number(
        acc.delayCount ?? acc.countofdelays ?? acc.countOfDelays ?? 0
      ),
    ...
    <div className="space-y-6">
        ...
        <div className="inline-flex rounded-xl bg-slate-100 p-1.5 border border-slate-200">
          {(['Active', 'Closed'] as const).map((segment) => (
            <button
              ...
              className={`rounded-lg px-6 py-2.5 text-sm font-bold transition-all ${activeSegment === segment ? 'bg-[#002366] text-white shadow-md' : 'text-slate-500 hover:text-slate-800'}`}
            >
              {segment} Accounts
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p
            className={`text-2xl font-extrabold mt-1 ${totals.overdue > 0 ? 'text-red-600' : 'text-slate-900'}`}
          >
                ...
                <div className="grid grid-cols-1 gap-4">
                  {typedAccounts.map((acc: any, index: number) => {
                    ...
                    return (
                      <div
                        key={uniqueId}
                        className={`overflow-hidden rounded-xl border transition-all ${hasOverdue ? 'border-red-200 shadow-red-50/50 shadow-sm' : 'border-slate-200 shadow-sm'} bg-white`}
                      >
                        {/* Main Summary Header */}
                        <div
                        ...
                        >
                          ...
                          <div className="flex items-start gap-3">
                            <div
                              className={`rounded-lg p-2 mt-0.5 ${hasOverdue ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-700'}`}
                            >
                          ...
                          <div className="flex flex-wrap md:flex-nowrap items-center gap-3 w-full md:w-auto justify-between md:justify-end">
                            ...
                            {acc.assetClassification && (
                              <span
                                className={`inline-flex items-center rounded-md border px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${getAssetBadgeClass(acc.assetClassification)}`}
                              >
                                {acc.assetClassification}
                              </span>
                            )}
                            ...
                            {isExpanded ? (
                              <ChevronUp className="h-5 w-5 text-slate-400 hidden md:block" />
                            ) : (
                              <ChevronDown className="h-5 w-5 text-slate-400 hidden md:block" />
                            )}
                          </div>
                        </div>
                              ...
                              <div className="space-y-2">
                                <div className="flex justify-between">
                                  ...
                                  <span className="font-bold text-slate-800">
                                    {formatINR(acc.sanctionedAmount)}
                                  </span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-slate-500 text-xs">
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex justify-between">
                                  ...
                                  <span
                                    className={`font-bold ${Number(acc.delayCount) > 0 ? 'text-red-500' : 'text-slate-700'}`}
                                  >
                                    {acc.delayCount}
                                  </span>
                                </div>
