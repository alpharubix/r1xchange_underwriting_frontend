...
export default function Step3OtpValidation({
  gstin,
  fromMonth,
  toMonth,
  onNext,
  ...
  const otpMutation = useMutation({
    mutationFn: (data: Parameters<typeof generateOtp>[0]) =>
      generateOtp(data, custId),
    onSuccess: (res) => {
    ...
    onError: (error: any) => {
      const msg =
        error.response?.data?.detail?.message || 'Failed to generate OTP';
      toast.error(msg);
    },
  });

  const validateMutation = useMutation({
    mutationFn: (data: Parameters<typeof validateOtp>[0]) =>
      validateOtp(data, custId),
    onSuccess: () => {
      setIsValidated(true);
  ...
  const submitMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(
        error.response?.data?.message || 'Failed to submit for analysis'
      );
    },
  });
    ...
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-sm border border-gray-100">
      <div className="flex items-center mb-6">
        <button
          ...
          disabled={isValidated || submitMutation.isPending}
        >
          â† Back
        </button>
      ...
      {!otpReferenceId ? (
        <form onSubmit={handleGenerateOtp} className="space-y-4">
          <p className="text-gray-500 text-sm mb-4">
            Please enter your username registered on the GST portal to trigger
            an OTP to your registered mobile number.
          </p>
        ...
        <form onSubmit={handleValidateOtp} className="space-y-4">
          <p className="text-gray-500 text-sm mb-4">
            An OTP has been sent to your registered mobile number for GSTIN{' '}
        ...
        <div className="space-y-4 text-center">
          <div className="bg-green-50 text-green-800 p-4 rounded-md text-sm mb-6 flex items-center border border-green-100">
            ...
            OTP verified. Click below to trigger the analysis for the selected
